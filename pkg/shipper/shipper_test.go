...
func TestShipperTimestamps(t *testing.T) {
	...
	s := New(nil, nil, dir, nil, nil, blockmeta.TestSource)
	...
	testutil.Ok(t, blockmeta.Write(log.NewNopLogger(), path.Join(dir, id1.String()), &blockmeta.Meta{
		Version: 1,
	...
	testutil.Ok(t, blockmeta.Write(log.NewNopLogger(), path.Join(dir, id2.String()), &blockmeta.Meta{
		Version: 1,
		BlockMeta: tsdb.BlockMeta{
			ULID:    id2,
			MaxTime: 4000,
